...
function transTimeUnit(data) {//h
        data = parseFloat(data);
        if (data == "") {
            return {data:0,unit:'3600'};
        }
	
        var vol = data;
        var result = {
			data : roundToTwoDecimalNumber(vol),
			unit : '3600'
		};
		
        if (vol < 1) {
            vol = data * 60;
            result.data = roundToTwoDecimalNumber(vol);
            result.unit = '60';
        }
        return result;
    }
 ...
 */
function checkCableMode(hasRJ45, currentMode) {
    if(hasRJ45){
