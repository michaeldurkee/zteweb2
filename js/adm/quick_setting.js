    ...
    function ($, ko, config, service, _) {
        ...
        function quickSettingVM() {
            ...
            self.apnDisabled = ko.computed(function () {
                return (info.apn_index < config.defaultApnSize || info.ppp_status == "ppp_connected" || info.ppp_status == "ppp_connecting");
            });

            self.apnModeDisabled = ko.computed(function () {
                return (info.ppp_status == "ppp_connected" || info.ppp_status == "ppp_connecting" || (!self.profileName() && self.apnMode() == apnMode.auto));//选项为空或正在连接或已连接
            });
            ...
            self.wifiDisabled = (info.wifi_cur_state != "1");
           ...
           self.selectedSecurityMode_trans = ko.computed(function () {
            ...
            self.next = function () {
                ...
                changeStep(toStep);
                if (currentStep == 1 && self.apnMode() == apnMode.auto) {
            ...
            self.previous = function () {
                ...
                var toStep = self.currentStep() - 1;
                changeStep(toStep);
                if (currentStep == 3 && self.apnMode() == apnMode.auto) {
                    toStep = self.currentStep() - 1;
                    changeStep(toStep);
                }
            };
            ...
            function checkWifiDisabled() {
                var data = service.getWpsInfo();
                if (data.wpsFlag == '1') {
                    return wifiState.wps_on;
                }else if(data.radioFlag == "0"){
