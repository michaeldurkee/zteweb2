...
define(['jquery', 'knockout', 'service', 'status/statusBar', 'echarts'], function ($, ko, service, status, echarts) {
    ...
    function TrafficVM() {
        ...
        self.dailyChecked = ko.observable(info.midnight_reboot_switch == '0' ? '0' : '1');
        ...
        self.TrafficResetHour  = ko.observable(info.sntp_daily_reset_hour);
        self.TrafficResetMin   = ko.observable(info.sntp_daily_reset_min);
        self.TrafficClearDay = ko.observable(info.sntp_reset_day);

        self.TrafficLastMonth = ko.observable(transUnit(parseInt(info.flux_last_month_total, 10), false));
...
//save used data when Data Type is time
        ...
        self.saveResetTime = function() {
            var requestParams = {
            goformId: "alk_set_daily_reset_time",
                sntp_daily_reset_hour:self.TrafficResetHour(),
                sntp_daily_reset_min: self.TrafficResetMin(),
                midnight_reboot_switch: self.dailyChecked()
          
            };

            service.setResetTime(requestParams, function(data) {
                if (data.result == "success") {
                    successOverlay();
        ...
        self.saveResetDay = function() {
            var requestParams = {
            goformId: "alk_set_flux_reset_date",
                sntp_reset_day: self.TrafficClearDay()
         
            };

            service.setResetDay(requestParams, function(data) {
                if (data.result == "success") {
                    successOverlay();
