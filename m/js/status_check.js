define(['service', 'config/config'], function(service, config) {
    ...
    function checkTrafficLimitAlert(info) {
        ...
        var APStationEnabled = config.AP_STATION_SUPPORT ? info.ap_station_enable : 'undefined';
        ...
        if (!info.isLoggedIn || inShow || (trafficAlertPopuped && trafficAlert100Popuped)
            || !info.limitVolumeEnable || (!APStationEnabled && !(info.connectStatus == "ppp_connected"))) {
            return false;
        }
