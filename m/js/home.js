define(['service', 'jquery', 'language', 'config/config'], function (service, $, language, config) {
    ...
    function refreshHome() {
		...
		if($("#signal_div").attr('src') != signalCSS){
			$("#signal_div").attr('src', signalCSS);
		}
        ...
        if (info.limitVolumeEnable && config.TRAFFIC_SUPPORT && !checkCableMode(config.RJ45_SUPPORT, config.DEVICE_MODE)) {
			...
			var timeMonth = transTimeUnit(trafficData.limitTimeMonth / 3600);
            homeUtil.setData('time_total', $.i18n.prop('time_total', timeMonth.data + $.i18n.prop(getTimeUnit(timeMonth.unit))));
        } else {
            homeUtil.hideElement('traffic_div');
        }

        homeUtil.setData('wireless_size', $.i18n.prop('n_wireless_devices', info.wirelessDeviceNum));
    ...
    var homeUtil = {
        ...
        getSignalCssClass: function (siganl, networkType, simStatus) {
            ...
            return "img/signal_" + siganl + ".png";
        }
    };

    function homeUlInit(){
        !config.HAS_SMS && $("#sms").css("display","none");
		!config.TRAFFIC_SUPPORT && $("#traffic").css("display","none");
    }
    ...
    function init() {
        ...
        service.getSmsCapability({}, function (capa) {
            smsUsed = capa.nvUsed + capa.simUsed;			
        });
		refreshHome();
        addInterval(function () {
            trafficData = service.getConnectionInfo();
            if(config.HAS_SMS){
                //smsUsed = service.getSmsCapability().nvUsed;
                service.getSmsCapability({}, function (capa) {
                    smsUsed = capa.nvUsed + capa.simUsed;			
                });
            }   
