define([ 'underscore', 'jquery', 'config/config'], function(_, $, config) {
    ...
    function getWifiBasic() {
        ...
        function prepare(params, isPost) {
            ...
            requestParams.cmd = "m_ssid_enable,wifi_cur_state,NoForwarding,m_NoForwarding," + wpapsk + "MAX_Station_num,"+
				...
				"DefaultKeyID,Key1Str1,Key2Str1,Key3Str1,Key4Str1,m_DefaultKeyID,m_Key1Str1,m_Key2Str1,m_Key3Str1,m_Key4Str1,wifi_sta_connection";
    ...
    function getApnSettings() {
		...
		function prepare(params, isPost) {
			...
			requestParams.cmd = "APN_config0,APN_config1,APN_config2,APN_config3,APN_config4,APN_config5,APN_config6,APN_config7,APN_config8,APN_config9," +
								...
								"ipv6_wan_apn,ipv6_pdp_type,ipv6_ppp_auth_mode,ipv6_ppp_username,ipv6_ppp_passwd";
			requestParams.multi_data = 1;
			return requestParams;
		}
		...
		function deal(result) {
			if (result) {
				return {
					...
					wanApn : result.wan_apn,
					authMode : result.ppp_auth_mode.toUpperCase(),//result.ppp_auth_mode.toLowerCase(),
					username : result.ppp_username,
					password : result.ppp_passwd,
					dnsMode : result.dns_mode,
					dns1 : result.prefer_dns_manual,
					dns2 : result.standby_dns_manual,
					wanApnV6 : result.ipv6_wan_apn,
					authModeV6 : result.ipv6_ppp_auth_mode.toUpperCase(),//result.ipv6_ppp_auth_mode.toLowerCase(),
					usernameV6 : result.ipv6_ppp_username,
					passwordV6 : result.ipv6_ppp_passwd,
					dnsModeV6 : "",
					dns1V6 : "",
	 ...
	 */
    var timerQueryString = [ "modem_main_state", "pin_status","blc_wan_auto_mode","blc_wan_mode","loginfo","new_version_state","current_upgrade_state","is_mandatory","sta_count", "m_sta_count"];
    var loginTimerQueryString = ["signalbar","network_type","sub_network_type", "network_provider",
        "ppp_status","EX_SSID1","ex_wifi_status","EX_wifi_profile","m_ssid_enable", "wifi_cur_state",
    ...
    function timerUpdateStatus(data) {
        ...
        if(data.blc_wan_mode == "AUTO"){
            config.DEVICE_MODE = data.blc_wan_auto_mode ? data.blc_wan_auto_mode : 'AUTO_PPP';
        } else{
            config.DEVICE_MODE = data.blc_wan_mode ? data.blc_wan_mode : 'PPP';
        }
        ...
        timerInfo.wifiStatus = data.wifi_cur_state == "1" ? "1" : "0";
        ...
        config.isLogin = timerInfo.isLoggedIn;
        if (config.HAS_SMS) {
    ...
    function getLoginData(){
        ...
        function prepare(params, isPost) {
            var requestParams = {};
            requestParams.isTest = isTest;
            requestParams.cmd = "modem_main_state,puknumber,pinnumber,psw_save,psw_fail_num_str,login_lock_time";
            requestParams.multi_data = 1;
            return requestParams;
        }
...
//                currentLoginStatus = undefined;
    ...
    function parseMessages(messages, isReport){
    	...
    	for(var i = 0 ; i < messages.length; i++){
            ...
            oneMessage.number8 = getLastNumber(messages[i].number, config.SMS_MATCH_LENGTH);
    ...
    function getWpsInfo() {
        ...
        function prepare(params, isPost) {
            var requestParams = {};
            requestParams.isTest = isTest;
            requestParams.cmd = "wifi_wps_index,WscModeOption,AuthMode,wifi_cur_state,EncrypType,wps_mode,WPS_SSID,m_ssid_enable,SSID1,m_SSID,m_EncrypType";
            requestParams.multi_data = 1;
            return requestParams;
        }
    ...
    function getQuickSettingInfo() {
        ...
        function prepare(params, isPost) {
            ...
            requestParams.cmd = "pdp_type,ipv6_pdp_type,wifi_cur_state,SSID1,HideSSID,AuthMode,WscModeOption,ppp_status,apn_index,ipv6_apn_index,ipv6_APN_index,m_profile_name,apn_mode" + wpask +
    ...
    function getDeviceInfo(){
        ...
        function prepare(params, isPost) {
            var requestParams = {
            	isTest: isTest,
                cmd : "wifi_coverage,m_ssid_enable,imei,network_type,sub_network_type,rssi,rscp,lte_rsrp,imsi,cr_version,wa_inner_version,hardware_version,MAX_Access_num," +
                		"SSID1,AuthMode,WPAPSK1_encode,m_SSID,m_AuthMode,m_HideSSID,m_WPAPSK1_encode,m_MAX_Access_num,lan_ipaddr," +
                		"mac_address,msisdn,LocalDomain,wan_ipaddr,static_wan_ipaddr,ipv6_wan_ipaddr,ipv6_pdp_type,pdp_type,ppp_status,ziccid",
                multi_data : 1
            };
            return requestParams;
        }
...
/**
    ...
    function getSignalStrength(){
        return getParams({nv: ['network_type','sub_network_type','lte_rssi', 'rssi', 'rscp', 'lte_rsrp']}, arguments[1], arguments[2]);
    }
    ...
    function trafficCalibration(){
        ...
        function prepare(params, isPost) {
            var requestParams = {
                ...
                calibration_way: params.way == '0'? 'time' : 'data',
                time: params.way == '0' ? params.timeAdjust : 0,
                data: params.way == '1' ? params.dataAdjust : 0
            };
            return requestParams;
        }
